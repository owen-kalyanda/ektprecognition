...
public class Crossings {
	...
	public void prosesCrossings(BufferedImage characterImg, JFrame frame2, JLabel lblChar, int countCross, JTextField tfCharacter, int numImg, int numImgForRow1 ){
		if (countCross > 0) {
			frame2.remove(lblChar);
		}
		...
		lblChar = new JLabel();
		lblChar.setBounds(500, 650, padImage2.getWidth(), padImage2.getHeight());
		lblChar.setIcon(new ImageIcon(padImage2));
		frame2.getContentPane().add(lblChar);
	    ...
	    String charToDB = tfCharacter.getText();
